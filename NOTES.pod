...
A HAL browser is bundled with the API and the supplied webapi-dbic-any.psgi.
...
Urgent:
    ...
    Fix auth for testing

    Fix missing _links in test schema
...
Things to do, or at least consider, before we stop being alpha:
    ...
    Review code for XXX / TODO markers
...
It's possibly better to use a surrogate key approach:
