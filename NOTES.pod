...
Things I (tim) plan to do in the first week of August if no one has got there first:

    Wrap RA-ChinookDemo example into a nice shell script
    Refactor key <-> id logic into a new ::Identity role
        - add support and tests for multiple keys
        - at least look into adding support for date/time keys (ie log table with id + dt keys)
        - at least look into adding support for alternate keys (re record vs resource)
    Refactor router interface, towards abstracting it to support others
    Refactor exception/error handling a little (see notes below)
    Add caching to relationship link creation
...
Here's a procedure to test WebAPI::DBIC with the schema/data from
RA::ChinookDemo (http://www.rapidapp.info/demos/chinook)
