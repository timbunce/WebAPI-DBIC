...
Things I (tim) plan to do in the first week of August if no one has got there first:
    ...
    Extend new ::Identity role
        - add support and tests for multiple simple keys
        - at least look into adding support for date/time keys (ie log table with id + dt keys)
        - at least look into adding support for alternate keys (re record vs resource) including PUT/POST
    Document ::Router interface, re abstracting it to support others
    ...
    Add tests for links being present and working
