...
sub _render_prefetch_jsonapi {
    ...
    while (my ($rel, $sub_rel) = each %{$prefetch}){
        ...
        if (not defined $subitem) {
            ...
            $data->{_embedded}{$rel} = $rel_set_resource->render_set_as_array_of_jsonapi_resource_objects($subitem);
        }
        else {
...
sub render_jsonapi_response { # return top-level document hashref
    ...
    my $set_data = $self->render_set_as_array_of_jsonapi_resource_objects($set);
